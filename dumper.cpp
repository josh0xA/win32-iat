...
BOOL DMP::Win32Dumper::Win32DumpExecutableInformation()
{
	...
	m_pImageHeader = reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<PCHAR>(m_pDosHeader) + m_pDosHeader->e_lfanew);
	...
	m_pImportDescriptor = reinterpret_cast<PIMAGE_IMPORT_DESCRIPTOR>(reinterpret_cast<PCHAR>(m_pDosHeader) + hd.Win32SetRvaToDwordOffset(
		m_pImageHeader, m_pDataDirectory->VirtualAddress));
	
	// HANDLE THUNK 32-BIT DATA
	m_pOriginalFirstThunk = reinterpret_cast<PIMAGE_THUNK_DATA32>(reinterpret_cast<PCHAR>(m_pDosHeader)
	...
	{
		m_dwName = reinterpret_cast<DWORD>(reinterpret_cast<PCHAR>(m_lpMap) + hd.Win32SetRvaToDwordOffset(m_pImageHeader, m_pImportDescriptor->Name));
		m_pOriginalFirstThunk = reinterpret_cast<PIMAGE_THUNK_DATA32>(reinterpret_cast<PCHAR>(m_pDosHeader) + hd.Win32SetRvaToDwordOffset(
			m_pImageHeader, m_pImportDescriptor->OriginalFirstThunk));

		m_pFirstThunk = reinterpret_cast<PIMAGE_THUNK_DATA32>(reinterpret_cast<PCHAR>(m_pDosHeader) + hd.Win32SetRvaToDwordOffset(m_pImageHeader,
			m_pImportDescriptor->FirstThunk));

		while (m_pOriginalFirstThunk->u1.AddressOfData != 0x00 && !m_isIATFound)
		{
			m_pNameImg = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(reinterpret_cast<PCHAR>(m_pDosHeader) + hd.Win32SetRvaToDwordOffset(m_pImageHeader,
